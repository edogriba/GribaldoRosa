...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textit{Void} sendNotificationToCompany(\textit{int} companyId);          % INT4
        \item \textit{Void} sendNotificationToStudent(\textit{Int} studentId);          % INT9
    \end{itemize}

    % - ProfileManager -  
    \item \textbf{ProfileManager}
    \begin{itemize}
        \item \textit{Dict} getProfileData(\textit{Int} userID);                            % INT3
        \item \textit{Bool} checkModifications(\textit{Dict} modifications);                % INT3
        \item \textit{Void} updateUserProfile(\textit{Dict} modifications);                 % INT3              
        \item \textit{List[Dict]} getApplicationListByStudent(\textit{Int} studentId);      % INT7
        \item \textit{List[Dict]} getApplicationListByInternshipPosition(\textit{Int} internshipPosition)   % INT13-14-15 16-17
        \item \textit{List[Dict]} getInternshipPositionList(\textit{Int} companyId);        % INT11
        \item \textit{List[Dict]} getInternshipListByCompany(\textit{Int} companyId);       % INT12
        \item \textit{List[Dict]} getInternshipListByUniversity(\textit{Int} universityId); % INT19  <--
    \end{itemize}

    % - ApplicationManager -
    \item \textbf{ApplicationManager}
    \begin{itemize}
        \item \textit{Bool} validateApplication(\textit{Bool} conditionsAccepted);  % INT6
        \item \textit{Void} writeNewApplication(\textit{Int} studentId, \textit{Int} internshipPositionId); % INT6
        \item \textit{Dict} getApplicationDetails(\textit{Int} applicationId);      % INT7
        \item \textit{Void} acceptApplication(\textit{Int} applicationId);          % INT14
        \item \textit{Void} rejectApplication(\textit{Int} applicationId);          % INT14
        \item \textit{Bool} checkInterviewData(\textit{Dict} interview);            % INT15
        \item \textit{Bool} checkFeedback(\textit{Dict} feedback);                  % INT16
        \item \textit{Void} sendFeedback(\textit{Dict} feedback);                   % INT16
        \item \textit{Void} sendMessage(\textit{Dict} message);                     % INT17
    \end{itemize}

    % - InternshipManager - 
    \item \textbf{InternshipManager}
    \begin{itemize}
        \item \textit{Bool} checkInternshipData(\textit{Dict} internshipData);                  % INT9 - INT10
        \item \textit{Void} writeNewInternship(\textit{Dict} internshipData);                   % INT9
        \item \textit{List[Int]} findMarchingStudents(\textit{Dict} internshipData);            % INT9
        \item \textit{Void} triggerNotification(\textit{Int} studentId);                        % INT9
        \item \textit{Void} closeInternshipById(\textit{Int} internshipId);                     % INT10
        \item \textit{Dict} getInternshipPositionDetails(\textit{Int} internshipPositionId);    % INT11
        \item \textit{Dict} getInternshipDetails(\textit{Int} internshipId);                    % INT12
        \item \textit{List[Dict]} getMarchedCandidates(\textit{Dict} internshipPosition);       % INT11
    \end{itemize}
\end{enumerate} 
