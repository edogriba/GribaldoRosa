...
class TestUniversity(unittest.TestCase):
    ...
    @patch.object(UniversityDB, 'update')
    @patch.object(UniversityDB, 'get_by_id', return_value={
        'id': 1,
        'email': "test@university.com",
        'password': "password",
    ...
    @patch.object(UniversityDB, 'update', side_effect=Exception("Database update error"))
    @patch.object(UniversityDB, 'close')
    def test_update_raises_exception(self, mock_close, mock_update):
        with self.assertRaises(Exception) as context:
            University.update(
                id=1,
                address="123 Updated St",
